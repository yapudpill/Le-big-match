...
with compatibilite as (
  ...
  select u.id, (
    ...
    select (select count(*) from I) / greatest((select count(*) from U), 1) as score_t
  )
  ...
  select u.id, (
    with T1 as (select evenement from presence where utilisateur = :'util'),
         ...
         I as ((select * from T1) intersect all (select * from T2)),
         U as ((select * from T1) union all (select * from T2))
  select (select count(*) from I) / greatest((select count(*) from U), 1) as score_e
  )
  from compatibilite u
...
select id, (0.5 * score_p + 0.3 * score_e + 0.2 * score_t) as score
from
  score_tags
